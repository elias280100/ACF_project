module tb_mult_4bit;
    ...
    task test_multiplication(input [3:0] a_val, b_val, input [7:0] expected);
        begin
            ...
            if (product == expected) begin
                $display("✓ PASS: %d × %d = %d (expected %d)", a_val, b_val, product, expected);
            end else begin
                $display("✗ FAIL: %d × %d = %d (expected %d)", a_val, b_val, product, expected);
            end
        end
    endtask
    ...
    // ========== Optional: Monitor all outputs ==========
    initial begin
        $monitor("Time=%0t | a=%4b (%2d) × b=%4b (%2d) = product=%8b (%3d)", 
